...
export function CopyField({label,value,variant="detail"}:{label:string;value:string;variant?:"name"|"bank"|"clabe"|"detail"}){
 ...
 if(variant==="name") return <div className={styles.nameLine}><h1 id="payment-title">{value}</h1><button className={styles.nameCopy} type="button" onClick={copy} aria-label="Copiar nombre completo"><CopyIcon/></button>{copied&&<span className={styles.inlineCopied}>Copiado</span>}</div>;
