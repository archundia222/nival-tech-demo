    ...
    <div className="dashboardContent">
      ...
      {!active ? <>
        <section className="productShowcase intelligenceShowcase">
          <div className="productShowcaseCopy"><span className="productPill">NIVAL INTELLIGENCE</span><h1>Deja de mirar datos.<br/>Empieza a entenderlos.</h1><p>Tu negocio genera señales todos los días. Intelligence conecta Nival Pay y Nival Puntos para decirte qué está pasando y qué puedes hacer después.</p><div className="productPrice"><strong>{hasPoints ? '$449' : '$399'}</strong><span>MXN / mes {hasPoints ? '· Puntos + Intelligence' : ''}</span></div><form action={startNivalIntelligenceSubscription}><button className="productCta">{hasPoints ? 'Activar el paquete completo' : 'Activar Nival Intelligence'} <span>→</span></button></form>{hasPoints && <small>Tu Nival Puntos queda incluido dentro de los $449 MXN mensuales.</small>}</div>
