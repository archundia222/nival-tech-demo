...
export default async function NivalIntelligencePage({ searchParams }: { searchParams: Promise<{ error?: string; subscription?: string }> }) {
  ...
  return <main className="dashboardApp">
    ...
    <div className="dashboardContent">
      <header className="dashboardContentTopbar"><div><span>Nival Intelligence</span><b>Pay + Puntos en un solo lugar</b></div><span className="ready">{active ? 'Activo' : '$1 de prueba'}</span></header>
      ...
      {!active ? <>
        <section className="productShowcase intelligenceShowcase">
          <div className="productShowcaseCopy"><span className="productPill">NIVAL INTELLIGENCE</span><h1>Deja de mirar datos.<br/>Empieza a entenderlos.</h1><p>Tu negocio genera señales todos los días. Intelligence conecta Nival Pay y Nival Puntos para decirte qué está pasando y qué puedes hacer después.</p><div className="productPrice"><strong>$1</strong><span>MXN / mes · precio de prueba</span></div><form action={startNivalIntelligenceSubscription}><button className="productCta">{hasPoints ? 'Activar el paquete completo' : 'Activar Nival Intelligence'} <span>→</span></button></form>{hasPoints && <small>Durante la prueba, el paquete completo también se cobra a $1 MXN.</small>}</div>
