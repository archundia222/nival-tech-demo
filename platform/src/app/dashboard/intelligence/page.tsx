  ...
  return <main className="dashboardApp nivalDashboard">
    ...
    <div className={`dashboardContent ${!active ? 'nivalIntelligenceDark' : ''}`}>
      ...
      {!active ? <>
        ...
        {view === 'overview' && <>
          <section className={styles.hero}><div><span className={styles.eyebrow}>TU PLAN DE HOY · 5 MINUTOS</span><h1>Haz esto hoy para traer clientes de vuelta.</h1><p>{atRisk.length ? `Hay ${atRisk.length} clientes en riesgo. ${recoverable.length} todavía están en una ventana clara de recuperación.` : frequent.length ? `Tienes ${frequent.length} clientes frecuentes. Hoy conviene fortalecer esa relación.` : 'Aún no hay una alerta urgente. Sigue registrando actividad para detectar oportunidades.'}</p><div className={styles.heroActions}><a href="#plan-hoy">Empezar plan de hoy</a><a href="/dashboard/intelligence?view=customers">Ver todos mis clientes</a></div></div><div className={styles.todayBadge}><small>PRIORIDAD DE HOY</small><strong>{atRisk.length ? 'Recuperar clientes' : frequent.length ? 'Fidelizar frecuentes' : 'Generar recurrencia'}</strong><span>Basado en la actividad registrada</span></div></section>
          ...
          <section className={styles.valueStrip}><div><small>CLIENTES QUE PUEDES RECUPERAR</small><strong>{contactableRisk.length}</strong><span>En riesgo y con contacto disponible</span></div><div><small>HÁBITOS QUE SE ESTÁN ROMPIENDO</small><strong>{slipping.length}</strong><span>Se retrasaron frente a su frecuencia normal</span></div><div><small>CLIENTES VALIOSOS EN RIESGO</small><strong>{frequentAtRisk.length}</strong><span>Ya habían visitado 3+ veces</span></div></section>
          <section className={styles.dailyPlan} id="plan-hoy">
            <div className={styles.planHeading}><div><span className={styles.label}>PLAN DE HOY</span><h2>3 pasos. Tú decides y Nival hace el análisis.</h2></div><span>{recoverable.length || frequent.length || newCustomers.length} clientes objetivo</span></div>
            <div className={styles.planGrid}><article><span className={styles.step}>PASO 1 · ELIGE A QUIÉN RECUPERAR</span><h3>{recoverable.length ? `Recupera a estos ${recoverable.length} clientes` : frequent.length ? `Premia a tus ${frequent.length} frecuentes` : `Convierte a ${newCustomers.length} nuevos en recurrentes`}</h3><p>{recoverable.length ? 'Llevan entre 31 y 60 días sin volver: todavía conocen tu negocio y son el grupo más lógico para contactar primero.' : frequent.length ? 'Ya demostraron que regresan. Una recompensa pequeña puede reforzar el hábito.' : 'El segundo regreso es la señal que empieza a convertir una visita aislada en relación.'}</p><a href="/dashboard/intelligence?view=customers">Ver lista de clientes →</a></article>
            <article className={styles.messageCard}><span className={styles.step}>PASO 2 · CONTÁCTALOS</span><h3>Mensaje listo para copiar</h3><blockquote>“{campaignMessage}”</blockquote><small>Cópialo, ajústalo si quieres y envíalo por tu canal habitual. Nival nunca contacta a tus clientes sin tu aprobación.</small></article>
            <article><span className={styles.step}>PASO 3 · REVISA SI REGRESARON</span><h3>Mide el resultado</h3><p>Registra las visitas después de la campaña. Intelligence podrá comparar cuántos regresaron y ajustar la siguiente recomendación.</p><div className={styles.planMetric}><b>{retention}%</b><span>retención observada hoy</span></div></article></div>
          </section>
          <section className={styles.strategyBoard}><div className={styles.planHeading}><div><span className={styles.label}>NO TIENES QUE ANALIZAR NADA</span><h2>Nival vigila estas señales mientras tú operas el negocio</h2></div><a href="/dashboard/intelligence?view=opportunities">Ver todas las oportunidades →</a></div><div className={styles.radarGrid}><article><span>URGENTE</span><h3>{frequentAtRisk.length ? `${frequentAtRisk.length} clientes frecuentes se están perdiendo` : 'Tus frecuentes están bajo control'}</h3><p>{frequentAtRisk.length ? 'Priorízalos sobre clientes nuevos: ya demostraron valor y ahora llevan más de 30 días sin regresar.' : 'No detectamos clientes de alta recurrencia fuera de su ventana reciente.'}</p></article><article><span>SEÑAL TEMPRANA</span><h3>{slipping.length ? `${slipping.length} clientes rompieron su ritmo habitual` : 'Sin cambios de hábito relevantes'}</h3><p>{slipping.length ? 'Todavía no son inactivos. Es el momento más temprano para intervenir antes de que se enfríen.' : 'Los clientes con frecuencia conocida siguen dentro de un comportamiento normal.'}</p></article><article><span>OPORTUNIDAD</span><h3>{frequent.length ? `${frequent.length} clientes merecen una estrategia VIP` : 'Construye tu primer grupo frecuente'}</h3><p>{frequent.length ? 'No les mandes la misma promoción que a todos. Reconoce su recurrencia y dales un motivo especial para mantenerla.' : 'Con más segundas y terceras visitas, Intelligence podrá detectar a tus mejores clientes.'}</p></article></div></section>
