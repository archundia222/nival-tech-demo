...
export default async function NivalIntelligencePage({ searchParams }: { searchParams: Promise<{ error?: string; subscription?: string; view?: string; campaign?: string; ticket?: string; question?: string; free?: string; saved?: string }> }) {
  ...
  const navActive = view === 'risk' ? 'inteligencia-riesgo' : view === 'recurring' ? 'inteligencia-recurrentes' : view === 'campaigns' ? 'inteligencia-campanas' : view === 'impact' ? 'inteligencia-impacto' : view === 'assistant' ? 'inteligencia-asistente' : 'inteligencia';

  const [{ data: customerRows }, { data: visitRows }, { data: rewardRows }, { data: campaignRows }] = available && hasPoints ? await Promise.all([
    ...
    <div className={`dashboardContent ${!available ? 'nivalIntelligenceDark' : ''}`}>
      ...
      {!hasPoints ? <>
        ...
        {view === 'overview' && <>
          ...
          <section className={styles.backgroundSignals}><div><span>En riesgo</span><b>{atRisk.length}</b></div><div><span>Frecuentes</span><b>{frequent.length}</b></div><div><span>Visitas · 30 días</span><b>{visits30}</b></div><div><span>Premios usados</span><b>{redeemedRewards}</b></div><p>Estas señales salen del uso normal de Nival Puntos: clientes registrados, visitas y recompensas. No necesitas capturar ventas para empezar a obtener recomendaciones.</p></section>
        </>}
        ...
        {view === 'impact' && <section className={styles.solutionWorkspace}><div className={styles.workspaceHero}><span className={styles.label}>RESULTADOS E IMPACTO</span><h1>¿Lo que hiciste con Nival trajo clientes de vuelta?</h1><p>Aquí solo mostramos resultados que podemos respaldar. Medimos regresos posteriores a la campaña; no los presentamos como ventas causadas por ella sin evidencia.</p></div>{params.campaign === 'started' && <div className={styles.successBanner}>Campaña registrada. Desde ahora Nival observará qué clientes de esa audiencia vuelven durante los próximos 30 días.</div>}{latestCampaign ? <div className={styles.campaignResult}><div><small>ÚLTIMA CAMPAÑA</small><strong>{latestCampaign.name}</strong><span>{latestCampaign.audience} clientes objetivo</span></div><div><small>REGRESARON DESPUÉS</small><strong>{latestCampaign.returned}</strong><span>{latestCampaign.rate}% de la audiencia</span></div><div><small>VALOR ESTIMADO</small><strong>{averageTicketCents ? `${(latestEstimatedValueCents / 100).toLocaleString('es-MX')}` : '—'}</strong><span>{averageTicketCents ? 'MXN usando tu ticket promedio' : 'Configura tu ticket promedio'}</span></div></div> : <div className={styles.emptyAction}>Aún no hay una campaña medida. Inicia una desde Campañas para comenzar a comparar resultados.</div>}<div className={styles.impactGrid}><article><small>RECURRENCIA OBSERVADA</small><strong>{retention}%</strong><p>Clientes registrados con dos o más visitas.</p></article><article><small>VISITAS · 30 DÍAS</small><strong>{visits30}</strong><p>{previous30 ? `${change >= 0 ? '+' : ''}${change}% frente a los 30 días anteriores.` : 'Aún sin periodo anterior comparable.'}</p></article><article><small>CLIENTES ACTIVOS</small><strong>{activeCustomers}</strong><p>Con una visita registrada en los últimos 30 días.</p></article></div><div className={styles.impactRoadmap}><div><span>1</span><b>Nival recomienda</b><small>Una campaña y audiencia concretas.</small></div><div><span>2</span><b>El negocio la aplica</b><small>Guardamos a quién se dirigió y cuándo.</small></div><div><span>3</span><b>Nival observa</b><small>Detectamos quién regresó después.</small></div><div><span>4</span><b>Nival aprende</b><small>Comparamos estrategias y recomendamos la siguiente.</small></div></div><div className={styles.ticketSetup}><div><b>Convierte regresos en un valor aproximado</b><p>Agrega tu ticket promedio y Nival estimará cuánto representan las visitas posteriores a una campaña. Se muestra como estimación, no como ingreso atribuido.</p></div><form action={saveAverageTicket}><label>Ticket promedio (MXN)<input name="averageTicket" inputMode="decimal" defaultValue={averageTicketCents ? (averageTicketCents / 100).toFixed(0) : ''} placeholder="Ej. 180" required/></label><button type="submit">Guardar</button></form></div><div className={styles.attributionNotice}><b>Qué falta para medir ingreso real</b><p>Cuando cada visita pueda incluir el valor real de compra, Intelligence podrá sustituir la estimación por dinero observado por cliente y campaña.</p></div></section>}
