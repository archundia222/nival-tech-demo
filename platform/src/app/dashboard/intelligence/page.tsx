...
export default async function NivalIntelligencePage({ searchParams }: { searchParams: Promise<{ error?: string; subscription?: string; view?: string }> }) {
  ...
  const navActive = view === 'customers' ? 'inteligencia-clientes' : view === 'imports' ? 'inteligencia-importar' : view === 'assistant' ? 'inteligencia-asistente' : 'inteligencia';
  ...
  return <main className="dashboardApp nivalDashboard">
    ...
    <div className={`dashboardContent ${!active ? 'nivalIntelligenceDark' : ''}`}>
      ...
      {!active ? <>
        ...
        {view === 'overview' && <>
          <section className={styles.hero}><div><span className={styles.eyebrow}>RESUMEN DEL NEGOCIO</span><h1>Hoy ya sabes qué necesita atención.</h1><p>{mainAdvice}</p></div><span className={styles.updated}>Datos actualizados al abrir el panel</span></section>
          <section className={styles.metrics}><article className={styles.card}><span>Clientes activos</span><strong>{activeCustomers}</strong><small>Con visita en los últimos 30 días</small></article><article className={styles.card}><span>Visitas en 30 días</span><strong>{visits30}</strong><small className={change >= 0 ? styles.good : styles.warn}>{change >= 0 ? '+' : ''}{change}% contra el periodo anterior</small></article><article className={styles.card}><span>Clientes en riesgo</span><strong>{atRisk.length}</strong><small>Más de 30 días sin volver</small></article><article className={styles.card}><span>Premios disponibles</span><strong>{availableRewards}</strong><small>Oportunidades de regreso</small></article></section>
          <section className={styles.grid}><article className={styles.panel}><div className={styles.panelHeader}><div><span className={styles.label}>CONSEJO DE HOY</span><h2>La acción con mayor prioridad</h2></div></div><div className={styles.insight}><span className={styles.label}>OPORTUNIDAD PRINCIPAL</span><h3>{mainAdvice}</h3><p>{atRisk.length ? 'Empieza por quienes ya tenían una relación con tu negocio. Contactarlos ahora suele ser más sencillo que conseguir clientes nuevos.' : 'Mientras reúnes más actividad, mantén constante el registro de visitas para detectar patrones útiles.'}</p><div className={styles.action}><span>Acción recomendada</span><b>{atRisk.length ? 'Revisar clientes en riesgo →' : 'Revisar registro de clientes →'}</b></div></div></article><aside className={styles.panel}><div className={styles.panelHeader}><div><span className={styles.label}>SEGMENTOS</span><h2>Clientes clave</h2></div><a href="/dashboard/intelligence?view=customers">Ver detalle</a></div><div className={styles.segments}><div className={styles.segment}><span>Frecuentes</span><b>{frequent.length}</b></div><div className={styles.segment}><span>En riesgo</span><b>{atRisk.length}</b></div><div className={styles.segment}><span>Nuevos</span><b>{customers.filter(c => c.visits <= 1).length}</b></div><div className={styles.segment}><span>Premio disponible</span><b>{availableRewards}</b></div></div></aside></section>
          <section className={styles.tools}><article className={styles.tool}><span className={styles.label}>DATOS</span><b>Importa clientes</b><p>Prepara la entrada de registros desde Excel, CSV u otra base de datos.</p><a href="/dashboard/intelligence?view=imports">Abrir centro de importación →</a></article><article className={styles.tool}><span className={styles.label}>ANÁLISIS</span><b>Clientes que podrían regresar</b><p>Revisa a quién conviene contactar y por qué.</p><a href="/dashboard/intelligence?view=customers">Ver clientes analizados →</a></article><article className={styles.tool}><span className={styles.label}>ASISTENTE</span><b>Pregunta sobre tu negocio</b><p>La base visual queda lista para consultar métricas y actividad conectada.</p><a href="/dashboard/intelligence?view=assistant">Abrir asistente →</a></article></section>
        </>}
