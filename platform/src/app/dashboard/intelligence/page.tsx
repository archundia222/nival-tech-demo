...
import { importCustomersCsv, importSalesCsv, registerDailySalesSummary, registerQuickSale } from '@/app/points/actions';
...
type Sale = { customer_id: string | null; amount_cents: number; sold_at: string; source: string; payment_method: string; transactions_count: number };
...
export default async function NivalIntelligencePage({ searchParams }: { searchParams: Promise<{ error?: string; subscription?: string; view?: string; campaign?: string; ticket?: string; question?: string; free?: string; saved?: string }> }) {
  ...
  const canManageGrowth = membership.role === 'owner' || membership.role === 'manager';
  ...
  const navActive = view === 'risk' ? 'inteligencia-riesgo' : view === 'inactive' ? 'inteligencia-inactivos' : view === 'recurring' ? 'inteligencia-recurrentes' : view === 'campaigns' ? 'inteligencia-campanas' : view === 'impact' ? 'inteligencia-impacto' : view === 'data' ? 'inteligencia-datos' : view === 'assistant' ? 'inteligencia-asistente' : 'inteligencia';
  ...
  const [{ data: customerRows }, { data: visitRows }, { data: rewardRows }, { data: campaignRows }, { data: saleRows }] = available && hasPoints ? await Promise.all([
    ...
    supabase.from('business_sales').select('customer_id,amount_cents,sold_at,source,payment_method,transactions_count').eq('business_id', membership.business_id).order('sold_at',{ascending:false}).limit(5000),
  ...
  const allSales = (saleRows ?? []) as Sale[];
  const linkedSales = allSales.filter((sale) => Boolean(sale.customer_id));
  const sales30 = allSales.filter((sale) => now - new Date(sale.sold_at).getTime() <= 30 * day);
  const localSaleDay = (iso: string) => {
  ...
  const detailedSaleDays = new Set(sales30.filter((sale) => sale.source !== 'summary').map((sale) => localSaleDay(sale.sold_at)));
  const effectiveSales30 = sales30.filter((sale) => sale.source !== 'summary' || !detailedSaleDays.has(localSaleDay(sale.sold_at)));
  const sales30Cents = effectiveSales30.reduce((sum,sale) => sum + Number(sale.amount_cents || 0),0);
  ...
  return <main className="dashboardApp nivalDashboard">
    ...
    <div className={`dashboardContent ${!available ? 'nivalIntelligenceDark' : ''}`}>
      ...
      {params.saved?.startsWith('import-') && <p className="formMessage successMessage">Ventas importadas correctamente.</p>}
      {params.saved?.startsWith('customers-') && <p className="formMessage successMessage">Clientes importados. Growth ya puede incorporarlos a sus análisis.</p>}
      {params.subscription && <p className={`formMessage ${paid ? 'successMessage' : ''}`}>{paid ? 'Suscripción confirmada. Nival Growth ya está activo.' : 'Estamos confirmando tu suscripción con Mercado Pago. No vuelvas a pagar mientras termina la validación.'}</p>}
      {subscriptionConfirming && <section className="subscriptionConfirming"><span>VALIDANDO GROWTH</span><strong>Mercado Pago ya devolvió el control a Nival.</strong><p>Estamos confirmando la suscripción y activaremos Growth automáticamente. No inicies otro pago mientras termina.</p></section>}
      {!hasPoints ? <>
        ...
        <section className="freemiumBanner">
          ...
          {!subscriptionConfirming && <form action={startNivalGrowthSubscription}><CheckoutSubmitButton className="nvPrimaryLink" pendingLabel="Abriendo Mercado Pago…">{growthCheckoutLabel} →</CheckoutSubmitButton></form>}
        </section>
        ...
        {view === 'impact' && <section className={styles.solutionWorkspace}><div className={styles.workspaceHero}><span className={styles.label}>RESULTADOS E IMPACTO</span><h1>¿Lo que hiciste con Nival trajo clientes de vuelta?</h1><p>Aquí separamos hechos de estimaciones: visitas posteriores, ventas registradas de la audiencia y valor aproximado cuando falta el monto real. Ninguno se presenta como causalidad sin evidencia.</p></div>{params.campaign === 'started' && <div className={styles.successBanner}>Campaña registrada. Desde ahora Nival observará qué clientes de esa audiencia vuelven durante los próximos 30 días.</div>}{latestCampaign ? <div className={styles.campaignResult}><div><small>ÚLTIMA CAMPAÑA</small><strong>{latestCampaign.name}</strong><span>{latestCampaign.audience} clientes objetivo</span></div><div><small>REGRESARON DESPUÉS</small><strong>{latestCampaign.returned}</strong><span>{latestCampaign.rate}% de la audiencia</span></div><div><small>{latestHasObservedSales ? 'VENTAS OBSERVADAS DESPUÉS' : 'VALOR ESTIMADO'}</small><strong>{latestHasObservedSales ? (latestObservedSalesCents / 100).toLocaleString('es-MX') : averageTicketCents ? (latestEstimatedValueCents / 100).toLocaleString('es-MX') : '—'}</strong><span>{latestHasObservedSales ? `MXN · ${latestCampaign.observedSalesCount} venta${latestCampaign.observedSalesCount === 1 ? '' : 's'} registrada${latestCampaign.observedSalesCount === 1 ? '' : 's'}` : averageTicketCents ? 'MXN · estimación con ticket promedio' : 'Registra ventas o configura tu ticket promedio'}</span></div></div> : <div className={styles.emptyAction}>Aún no hay una campaña medida. Inicia una desde Campañas para comenzar a comparar resultados.</div>}<div className={styles.impactGrid}><article><small>RECURRENCIA OBSERVADA</small><strong>{retention}%</strong><p>Clientes registrados con dos o más visitas.</p></article><article><small>VISITAS · 30 DÍAS</small><strong>{visits30}</strong><p>{previous30 ? `${change >= 0 ? '+' : ''}${change}% frente a los 30 días anteriores.` : 'Aún sin periodo anterior comparable.'}</p></article><article><small>VENTAS VINCULADAS</small><strong>{linkedSales.length}</strong><p>Registros con cliente identificado que Growth puede cruzar con campañas.</p></article></div><div className={styles.impactRoadmap}><div><span>1</span><b>Nival recomienda</b><small>Una campaña y audiencia concretas.</small></div><div><span>2</span><b>El negocio la aplica</b><small>Guardamos a quién se dirigió y cuándo.</small></div><div><span>3</span><b>Nival observa</b><small>Detectamos visitas y ventas posteriores de esa audiencia.</small></div><div><span>4</span><b>Nival aprende</b><small>Comparamos estrategias y recomendamos la siguiente.</small></div></div>{canManageGrowth && <div className={styles.saleCapture}><div><span className={styles.label}>REGISTRAR VENTA REAL</span><b>Conecta dinero con comportamiento</b><p>Cuando conozcas al cliente que compró, registra el monto. Growth podrá cruzar esa venta con visitas y campañas sin inventar atribución.</p></div><form action={registerQuickSale}><input type="hidden" name="returnTo" value="/dashboard/intelligence?view=impact"/><label>Cliente<select name="customerId" required defaultValue=""><option value="" disabled>Selecciona cliente</option>{customers.slice(0,200).map(customer=><option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label><label>Monto (MXN)<input name="amount" inputMode="decimal" placeholder="Ej. 180" required/></label><label>Método<select name="paymentMethod" defaultValue="other"><option value="cash">Efectivo</option><option value="transfer">Transferencia</option><option value="card">Tarjeta</option><option value="other">Otro</option></select></label><button type="submit">Guardar venta</button></form></div>}<div className={styles.ticketSetup}><div><b>Usa el ticket promedio como respaldo, no como verdad</b><p>Cuando una venta está vinculada al cliente, Growth usa el monto real registrado. Si no lo está, el ticket promedio sirve únicamente para estimar el valor de los regresos.</p></div>{canManageGrowth ? <form action={saveAverageTicket}><label>Ticket promedio (MXN)<input name="averageTicket" inputMode="decimal" defaultValue={averageTicketCents ? (averageTicketCents / 100).toFixed(0) : ''} placeholder="Ej. 180" required/></label><button type="submit">Guardar</button></form> : <span className={styles.readOnlyNote}>Solo propietario o gerente puede cambiar este valor.</span>}</div><div className={styles.attributionNotice}><b>Qué significa “ventas observadas después”</b><p>Son compras registradas de clientes que estaban en la audiencia y ocurrieron después del inicio de medición. Nival no afirma que la campaña las haya causado; las muestra como señal temporal verificable.</p></div></section>}
        {view === 'data' && <section className={styles.dataWorkspace}>
          <div className={styles.workspaceHero}><span className={styles.label}>VENTAS Y DATOS</span><h1>Alimenta Growth sin convertir tu negocio en una oficina.</h1><p>Registra solo lo que realmente aporta señal: ventas, clientes y resúmenes. Puedes empezar manualmente y después importar lo que ya tengas.</p></div>
          <div className={styles.dataSummary}><div><small>REGISTROS · 30 DÍAS</small><strong>{effectiveSales30.length}</strong><span>{(sales30Cents/100).toLocaleString('es-MX')} MXN sin duplicar resúmenes</span></div><div><small>VENTAS CON CLIENTE</small><strong>{linkedSales.length}</strong><span>pueden cruzarse con campañas</span></div><div><small>CLIENTES</small><strong>{customers.length}</strong><span>disponibles para análisis</span></div><div><small>VISITAS</small><strong>{visits.length}</strong><span>señal de recurrencia</span></div></div>
          {canManageGrowth ? <>
          <div className={styles.dataGrid}>
            <article className={styles.dataCard}><span className={styles.label}>REGISTRO RÁPIDO</span><h2>Venta con cliente</h2><p>Úsalo cuando sabes quién compró. Es la mejor señal para relacionar dinero con visitas y campañas.</p><form action={registerQuickSale}><input type="hidden" name="returnTo" value="/dashboard/intelligence?view=data"/><label>Cliente<select name="customerId" required defaultValue=""><option value="" disabled>Selecciona cliente</option>{customers.slice(0,200).map(customer=><option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label><label>Monto (MXN)<input name="amount" inputMode="decimal" placeholder="Ej. 180" required/></label><label>Método<select name="paymentMethod" defaultValue="other"><option value="cash">Efectivo</option><option value="transfer">Transferencia</option><option value="card">Tarjeta</option><option value="other">Otro</option></select></label><button type="submit">Guardar venta</button></form></article>
            <article className={styles.dataCard}><span className={styles.label}>CIERRE DEL DÍA</span><h2>Resumen diario</h2><p>Si no quieres capturar venta por venta, guarda el total del día. Sirve para tendencia general aunque no pueda atribuirse a clientes.</p><form action={registerDailySalesSummary}><input type="hidden" name="returnTo" value="/dashboard/intelligence?view=data"/><label>Fecha<input name="saleDate" type="date" required/></label><label>Total vendido (MXN)<input name="amount" inputMode="decimal" placeholder="Ej. 4250" required/></label><label>Número de ventas<input name="transactions" inputMode="numeric" type="number" min="1" defaultValue="1" required/></label><button type="submit">Guardar resumen</button></form></article>
          </div>
          <div className={styles.importGrid}>
            <article><span className={styles.label}>IMPORTAR CLIENTES</span><h3>Trae tu base existente</h3><p>CSV con Nombre y al menos Teléfono o Correo. Nival evita crear duplicados obvios y respeta el consentimiento indicado en el archivo.</p><form action={importCustomersCsv}><input type="hidden" name="returnTo" value="/dashboard/intelligence?view=data"/><input name="customersFile" type="file" accept=".csv,text/csv" required/><label className={styles.dataConsent}><input name="dataAuthorization" type="checkbox" required/> Confirmo que el negocio puede utilizar estos datos.</label><button type="submit">Importar clientes</button></form></article>
            <article><span className={styles.label}>IMPORTAR VENTAS</span><h3>Usa lo que ya registras</h3><p>CSV con Monto/Total; opcionalmente Fecha, Teléfono y Método. Si el teléfono coincide con un cliente, Growth vincula la venta automáticamente.</p><form action={importSalesCsv}><input type="hidden" name="returnTo" value="/dashboard/intelligence?view=data"/><input name="salesFile" type="file" accept=".csv,text/csv" required/><label className={styles.dataConsent}><input name="dataAuthorization" type="checkbox" required/> Confirmo que puedo utilizar la información de este archivo.</label><button type="submit">Importar ventas</button></form></article>
          </div>
          </> : <div className={styles.emptyAction}>Solo el propietario o un gerente puede registrar e importar datos financieros. Puedes seguir consultando los resultados del negocio.</div>}
          <div className={styles.dataPrinciple}><b>No necesitas capturarlo todo.</b><p>Growth funciona mejor con datos consistentes que con una base enorme y desordenada. Empieza por visitas y ventas vinculadas a clientes; usa resúmenes diarios para completar la tendencia del negocio.</p></div>
        </section>}
        {view === 'assistant' && <section className={styles.assistantWorkspace}>
          <div className={styles.assistantHero}>
            <div><span className={styles.label}>ASISTENTE GROWTH · BETA</span><h1>Pregunta por tu negocio, no por una tabla.</h1><p>Nival cruza clientes, visitas, campañas y ventas registradas para darte una respuesta accionable. La versión actual usa análisis estructurado; la arquitectura ya está preparada para conectar IA generativa sin cambiar tus datos.</p></div>
            <div className={styles.assistantTrust}><small>CONTEXTO DISPONIBLE</small><strong>{customers.length} clientes · {visits.length} visitas</strong><span>{linkedSales.length} ventas vinculadas · {campaignStats.length} campañas medidas</span></div>
          </div>
          <div className={styles.assistantShell}>
            <article className={styles.assistantConversation}>
              <div className={styles.assistantPromptIdeas}>
                <span>PREGUNTAS ÚTILES</span>
                <div>
                  <a href="/dashboard/intelligence?view=assistant&question=%C2%BFA+qui%C3%A9n+estoy+perdiendo%3F">¿A quién estoy perdiendo?</a>
                  <a href="/dashboard/intelligence?view=assistant&question=%C2%BFQu%C3%A9+campa%C3%B1a+har%C3%ADas+hoy%3F">¿Qué campaña harías hoy?</a>
                  <a href="/dashboard/intelligence?view=assistant&question=%C2%BFQui%C3%A9nes+son+mis+mejores+clientes%3F">¿Quiénes son mis mejores clientes?</a>
                  <a href="/dashboard/intelligence?view=assistant&question=%C2%BFFuncion%C3%B3+mi+%C3%BAltima+campa%C3%B1a%3F">¿Funcionó mi última campaña?</a>
                  <a href="/dashboard/intelligence?view=assistant&question=%C2%BFCu%C3%A1nto+dinero+representaron+los+regresos%3F">¿Cuánto dinero representaron los regresos?</a>
                </div>
              </div>
              <form className={styles.chat} method="get"><input type="hidden" name="view" value="assistant"/><input name="question" aria-label="Pregunta para Nival" defaultValue={params.question ?? ''} placeholder="Ej. ¿Qué debería hacer hoy para recuperar clientes?"/><button type="submit">Analizar</button></form>
              {params.question ? <div className={styles.assistantThread}><div className={styles.userBubble}><small>TÚ</small><p>{params.question}</p></div>{assistantAnswer && <div className={styles.assistantAnswer}><small>NIVAL · BASADO EN TUS DATOS</small><p>{assistantAnswer}</p><div className={styles.answerEvidence}><span>{customers.length} clientes analizados</span><span>{visits30} visitas · 30 días</span><span>{linkedSales.length} ventas vinculadas</span></div></div>}</div> : <div className={styles.assistantEmpty}><strong>Empieza con una decisión real.</strong><p>Pregunta por clientes, campañas, recurrencia o dinero. Nival evita responder con datos que no existen en tu negocio.</p></div>}
              <p className={styles.sourceNote}>Beta actual: análisis determinista con datos del negocio. La capa de contexto ya está preparada para un modelo de IA; ventas posteriores a campañas se muestran como observadas, no como causalidad.</p>
            </article>
            <aside className={styles.assistantSide}>
              <span className={styles.label}>LO QUE NIVAL ESTÁ VIENDO</span>
              <div className={styles.segments}><div className={styles.segment}><span>En riesgo</span><b>{atRisk.length}</b></div><div className={styles.segment}><span>Inactivos</span><b>{inactive.length}</b></div><div className={styles.segment}><span>Frecuentes</span><b>{frequent.length}</b></div><div className={styles.segment}><span>Campañas medidas</span><b>{campaignStats.length}</b></div></div>
              <div className={styles.assistantGuardrails}><b>Qué sí hará la IA</b><p>Priorizar, explicar patrones, redactar mensajes y comparar acciones con tus datos.</p><b>Qué no debe hacer</b><p>Inventar clientes, atribuir ventas sin evidencia ni mandar campañas sin aprobación.</p></div>
            </aside>
          </div>
        </section>}
      </div>}
