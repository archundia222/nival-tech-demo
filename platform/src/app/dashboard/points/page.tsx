...
export default async function NivalPointsPage({ searchParams }: { searchParams: Promise<{ error?: string; subscription?: string }> }) {
  ...
  return <main className="dashboardApp">
    ...
    <div className="dashboardContent">
      <header className="dashboardContentTopbar"><div><span>Nival Puntos</span><b>Lealtad y recompensas</b></div><span className="ready">{active ? 'Activo' : '$10 de prueba'}</span></header>
      ...
      {!active ? <>
        <section className="productShowcase pointsShowcase">
          <div className="productShowcaseCopy"><span className="productPill">NIVAL PUNTOS</span><h1>Haz que tus clientes<br/>quieran volver.</h1><p>Premia cada visita con puntos. Tus clientes ven su saldo desde el celular y tú administras todo sin tarjetas de papel.</p><ul className="productBenefits"><li>Registro con código QR</li><li>Puntos y premios automáticos</li><li>Historial de clientes y visitas</li></ul><div className="productPrice"><strong>$10</strong><span>MXN al mes · prueba</span></div><form action={startNivalPointsSubscription}><button className="productCta">Comenzar por $10 al mes <span>→</span></button></form><small>Puedes cancelar cuando quieras. El cobro se realiza cada mes.</small></div>
