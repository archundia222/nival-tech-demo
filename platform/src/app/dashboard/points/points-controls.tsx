...
export function PointsProgramForm({ program }: { program: { name: string; reward_threshold: number; reward_description: string; point_cooldown_minutes: number; daily_points_cap: number; review_url?: string | null; review_request_visit?: number | null } }) {
  ...
  return <form className="pointsProgramForm" action={(formData) => startTransition(async () => {
    ...
    <div className="pointsFormGrid">
      ...
      <label>Tiempo de espera entre puntos<select name="cooldown" defaultValue={String(program.point_cooldown_minutes)}><option value="0">Sin tiempo de espera</option><option value="5">5 minutos</option><option value="10">10 minutos</option><option value="15">15 minutos</option><option value="30">30 minutos</option><option value="60">1 hora</option><option value="120">2 horas</option><option value="240">4 horas</option><option value="480">8 horas</option><option value="720">12 horas</option><option value="1440">24 horas</option>{![0,5,10,15,30,60,120,240,480,720,1440].includes(program.point_cooldown_minutes) && <option value={program.point_cooldown_minutes}>{program.point_cooldown_minutes} minutos (actual)</option>}</select><small>“Sin tiempo de espera” permite registrar otra visita inmediatamente.</small></label>
    </div>
