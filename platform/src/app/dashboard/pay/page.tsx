...
export default async function PaySettings({ searchParams }: { searchParams: Promise<{ profile?: string; new?: string; error?: string; view?: string; unlocked?: string; result?: string; created?: string }> }) {
    ...
    <div className="dashboardContent dashboardPayContent">
      ...
      <section className="dashboardHero">
        <div>
          ...
          <p>Recibe transferencias con tarjeta NFC, enlace y QR propios. Puedes agregar apartados opcionales por $10 MXN cada uno.</p>
  ...
  return <main className="dashboardApp">
    ...
    <div className="dashboardContent dashboardPayContent">
      ...
      {currentView === 'manage' && <><header className="payHeading payManageHeading"><p className="eyebrow">NIVAL PAY</p><h1>Tu página de cobro.</h1><p>Edita una sola vez lo que verán tus clientes desde la tarjeta NFC, el enlace y el QR.</p></header><form className="cardSelector cardSelectorRefined" method="get"><label><span>Nival Pay seleccionada</span><select name="profile" defaultValue={profile?.id}>{profiles?.map(item => <option key={item.id} value={item.id}>{item.display_name}</option>)}</select></label><button className="primaryButton">Abrir</button></form>{['owner','manager'].includes(membership.role) ? profile && <PaymentEditor key={profile.id} businessId={membership.business_id} businessName={business?.name ?? 'Mi negocio'} businessLogo={business?.logo_url ?? null} profile={profile} siteUrl={publicSiteUrl()} /> : <p>Solo el propietario o un administrador puede configurar Nival Pay.</p>}</>}
