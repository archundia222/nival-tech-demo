...
{sections.map((section, index) => <div className="customPaySection" key={section.id} style={{position:"relative",paddingTop:"3.2rem"}}>
        ...
        <div className="inlineApartados">
          <p className="apartadoIntro">Agrega los apartados que necesites. <strong>Precio temporal de prueba: $1 MXN</strong> y queda ligado a esta Nival Pay.</p>
          {sections.length < totalSectionLimit ? <button key="add-available-section" type="button" className="apartadoRowAdd" onClick={(event) => { event.preventDefault(); event.stopPropagation(); addSection(); }}><span><b>+</b></span><div><strong>Crear apartado comprado</strong><small>{totalSectionLimit-sections.length} disponible</small></div></button> : <button key="buy-extra-section" type="button" onClick={buyExtraSection} disabled={checkoutPending || !profile?.id} className="apartadoRowAdd apartadoRowLocked"><span><b>+</b></span><div><strong>{checkoutPending ? 'Abriendo Mercado Pago…' : 'Agregar apartado · $1 MXN (prueba)'}</strong><small>Paga una sola vez y edítalo cuando quieras.</small></div></button>}
