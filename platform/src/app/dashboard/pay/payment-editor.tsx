...
type Profile = { account_holder: string; bank_name: string; clabe: string; payment_url: string | null; image_url: string | null; public_token: string; active: boolean; view_count: number };
  ...
  return <>
    <div className="payEditorGrid">
      ...
      <aside className="payPreview"><p className="eyebrow">VISTA PREVIA</p><div className="payPreviewCard">
        {image ? <Image src={image} width={88} height={88} unoptimized alt={`Imagen de ${businessName}`} /> : <div className="payMonogram">{businessName.slice(0,1)}</div>}
        <h2>{businessName}</h2><p>Datos para transferencia</p>
        <dl><dt>Titular</dt><dd>{holder || 'Nombre del titular'}</dd><dt>Banco</dt><dd>{bank || 'Tu banco'}</dd><dt>CLABE</dt><dd>{clabe || '•••• •••• •••• •••• ••'}</dd></dl>
