...
export default async function SalesAdmin({ searchParams }: { searchParams: Promise<{ error?: string; section?: string }> }) {
  ...
  const { data: physicalCards } = await admin.from('physical_card_orders')
    .select('id, product_order_id, business_id, front_template, back_style, design, design_notes, back_design_notes, back_design_url, target_url, recipient_name, phone, delivery_method, requested_delivery_date, address_line1, address_line2, city, state, postal_code, fulfillment_status, tracking_code, created_at, businesses(name), product_orders!physical_card_orders_product_order_id_fkey(status, payment_method, amount_cents)')
