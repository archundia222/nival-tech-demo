  ...
  return <main className="pointsCustomerShell nivalDashboard">
    ...
    <details className="pointsCustomerRewards">
      ...
      <div className="pointsRewardList">
        ...
        {availableRewards.length ? availableRewards.map((reward: { id: string; description: string; earned_at: string }) => <article key={reward.id}><strong>{reward.description}</strong><span>Disponible · obtenida {new Intl.DateTimeFormat("es-MX",{dateStyle:"medium",timeZone:"America/Mexico_City"}).format(new Date(reward.earned_at))}</span><p>Muestra tu QR temporal en caja para canjearla.</p></article>) : <p className="pointsMuted">Todavía no tienes recompensas disponibles.</p>}
        <h2>Historial</h2>
