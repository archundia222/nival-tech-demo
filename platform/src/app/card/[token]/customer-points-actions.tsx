...
export function CustomerPointsActions({accountToken,rewards,pointsRemaining}:{accountToken:string;rewards:Reward[];pointsRemaining:number}){
 ...
 const toggle=(next:"points"|"rewards")=>{setPanel(panel===next?null:next);setRedeemOpen(false)};
 return <section className="pointsCustomerActions">
  ...
  {panel==="rewards"&&<div className="pointsActionPanel rewards">
   <div className="pointsRewardsHeader"><div><span>MIS RECOMPENSAS</span><h2>{available.length?available.length===1?"Tienes 1 premio listo":`Tienes ${available.length} premios listos`:"Tu espacio de recompensas"}</h2><p>{available.length?"Tus premios están guardados hasta que decidas usarlos.":"Aquí aparecerán los premios que vayas desbloqueando."}</p></div><div className={"pointsRewardsCount "+(available.length?"ready":"")}><strong>{available.length}</strong><span>disponibles</span></div></div>
   {available.length>0?<div className="pointsRewardWallet">{available.map((r,i)=><article key={r.id} className="pointsRewardTicket"><div className="pointsRewardTicketIcon">★</div><div><small>RECOMPENSA {i+1}</small><strong>{r.description}</strong><span>Desbloqueada el {date(r.earned_at)}</span></div><b>LISTA</b></article>)}</div>:<div className="pointsRewardsEmpty"><span>☆</span><strong>Sigue sumando visitas</strong><p>Te faltan {pointsRemaining} puntos para desbloquear tu próxima recompensa.</p></div>}
   {available.length>0&&!redeemOpen&&<button className="pointsOpenRedeem" type="button" onClick={()=>setRedeemOpen(true)}><span><b>Canjear recompensa</b><small>Generar QR y código temporal</small></span><strong>→</strong></button>}
   {available.length>0&&redeemOpen&&<div className="pointsRedeemPanel"><div className="pointsRedeemPanelTop"><div><span>CÓDIGO DE CANJE</span><strong>Muéstralo solamente al momento de recibir tu premio</strong></div><button type="button" onClick={()=>setRedeemOpen(false)} aria-label="Cerrar código de canje">×</button></div><RotatingPointsQr accountToken={accountToken} purpose="redeem"/></div>}
   <details className="pointsRewardHistory"><summary><span>Historial</span><b>{redeemed.length}</b></summary><div>{redeemed.length?redeemed.map(r=><article key={r.id}><span className="pointsHistoryCheck">✓</span><div><strong>{r.description}</strong><small>Canjeada el {date(r.redeemed_at!)}</small></div></article>):<p>Aún no has utilizado ninguna recompensa.</p>}</div></details>
  </div>}
 </section>;
}