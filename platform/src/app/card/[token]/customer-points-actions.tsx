...
export function CustomerPointsActions({accountToken,rewards,pointsRemaining}:{accountToken:string;rewards:Reward[];pointsRemaining:number}){
 ...
 const toggle=(next:"points"|"rewards")=>setPanel(panel===next?null:next);
 return <section className="pointsCustomerActions">
  ...
  {panel==="rewards"&&<div className="pointsActionPanel rewards">
   ...
   {available.length>0?<><div className="pointsRewardWallet">{available.map((r,i)=><article key={r.id} className="pointsRewardTicket"><div className="pointsRewardTicketIcon">★</div><div><small>RECOMPENSA {i+1}</small><strong>{r.description}</strong><span>Desbloqueada el {date(r.earned_at)}</span></div><span>LISTA</span></article>)}</div><p className="pointsRewardChooseHint">Para usar un premio, muestra tu tarjeta de Google Wallet en caja. Al registrar tu visita, Nival avisará al negocio y podrás decidir si canjearlo ahora o guardarlo.</p></>:<div className="pointsRewardsEmpty"><span>☆</span><strong>Sigue sumando visitas</strong><p>Te faltan {pointsRemaining} puntos para desbloquear tu próxima recompensa.</p></div>}
