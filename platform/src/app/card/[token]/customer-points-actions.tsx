...
export function CustomerPointsActions({accountToken,rewards,pointsRemaining}:{accountToken:string;rewards:Reward[];pointsRemaining:number}){
 ...
 return <section className="pointsCustomerActions">
  ...
  {panel==="rewards"&&<div className="pointsActionPanel rewards">
   ...
   {available.length>0?<><div className="pointsRewardWallet">{available.map((r,i)=><button key={r.id} type="button" className="pointsRewardTicket" onClick={()=>setSelectedRewardId(r.id)}><div className="pointsRewardTicketIcon">★</div><div><small>RECOMPENSA {i+1}</small><strong>{r.description}</strong><span>Desbloqueada el {date(r.earned_at)}</span></div><span>{selectedRewardId===r.id?"SELECCIONADA":"CANJEAR"}</span></button>)}</div><p className="pointsRewardChooseHint">Toca la recompensa que quieras usar. Puedes canjearla cuando quieras, sin registrar una visita nueva.</p>{selectedRewardId&&<div className="pointsRedeemPanel"><div className="pointsRedeemPanelTop"><div><span>CÓDIGO DE CANJE</span><strong>{available.find(r=>r.id===selectedRewardId)?.description}</strong><small>Muéstralo en caja cuando quieras usar este premio.</small></div><button type="button" onClick={()=>setSelectedRewardId(null)} aria-label="Cerrar código de canje">×</button></div><RotatingPointsQr accountToken={accountToken} purpose="redeem" rewardId={selectedRewardId}/></div>}</>:<div className="pointsRewardsEmpty"><span>☆</span><strong>Sigue sumando visitas</strong><p>Te faltan {pointsRemaining} puntos para desbloquear tu próxima recompensa.</p></div>}
