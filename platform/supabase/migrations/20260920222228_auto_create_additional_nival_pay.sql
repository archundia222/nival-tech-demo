...
create unique index if not exists payment_profiles_source_order_id_key
  on public.payment_profiles (source_order_id)
  where source_order_id is not null;

comment on column public.payment_profiles.source_order_id is
  'Paid product order that automatically created this additional Nival Pay profile.';
...
begin
  ...
  if v_order.product_code = 'nival_pay_extra_section' then
    ...
    where payment_profiles.business_id = v_order.business_id
    order by created_at
    limit 1;
    ...
    insert into public.payment_profiles (
      ...
      v_source.payment_url_visible, 'Nival Pay ' || v_profile_number,
      true, v_order.id
    )
    on conflict (source_order_id) where source_order_id is not null do nothing;

  elsif v_order.product_code = 'nival_pay_physical_card' then
    null;
  else
...
revoke all on function public.finalize_nival_pay_order(uuid, text)
  from public, anon, authenticated;
grant execute on function public.finalize_nival_pay_order(uuid, text)
  to service_role;
