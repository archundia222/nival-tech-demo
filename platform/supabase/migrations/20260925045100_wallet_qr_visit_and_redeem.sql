create or replace function public.claim_wallet_loyalty_card(
...
begin
  ...
  if p_purpose not in ('visit','redeem') then raise exception 'invalid_scan_purpose'; end if;
  ...
  if v_account.id is null then raise exception 'wallet_card_not_available'; end if;

  select lp.* into v_program
  from public.loyalty_programs lp
  where lp.id = v_account.program_id and lp.active = true;

  if v_program.id is null then raise exception 'points_program_unavailable'; end if;

  if p_purpose = 'redeem' then
    select r.* into v_reward
    from public.loyalty_rewards r
    where r.loyalty_account_id = v_account.id
      and r.redeemed_at is null
    order by r.earned_at asc, r.id asc
    limit 1
    for update;

    if v_reward.id is null then raise exception 'no_available_reward'; end if;
  end if;

  insert into public.loyalty_scan_tokens(
    business_id, program_id, loyalty_account_id, customer_id,
    token_hash, expires_at, used_at, used_by, purpose, reward_id
  )
  values(
    ...
    v_exp, now(), auth.uid(), p_purpose,
    case when p_purpose='redeem' then v_reward.id else null end
  )
  returning id into v_token_id;

  insert into public.loyalty_scan_sessions(
    business_id, program_id, loyalty_account_id, customer_id,
    employee_user_id, scan_token_id, expires_at, purpose, reward_id
  )
  values(
    v_account.business_id, v_account.program_id, v_account.id, v_account.customer_id,
    auth.uid(), v_token_id, v_exp, p_purpose,
    case when p_purpose='redeem' then v_reward.id else null end
  )
  ...
  select
    ...
    case when p_purpose='redeem' then v_reward.description else v_program.reward_description end,
    v_exp,
...
revoke all on function public.claim_wallet_loyalty_card(uuid,text) from public;
revoke all on function public.claim_wallet_loyalty_card(uuid,text) from anon;
