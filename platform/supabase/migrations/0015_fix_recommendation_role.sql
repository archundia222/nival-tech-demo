-- Preserve the tested role fix in migration history.
...
declare
  ...
  v_member_role public.business_role;
  segments record;
  generated_count integer := 0;
  visit_change integer := 0;
begin
  select bm.business_id, bm.role
  into current_business_id, v_member_role
  from public.business_members bm
  where bm.user_id = auth.uid()
  order by case when bm.role = 'owner' then 0 when bm.role = 'manager' then 1 else 2 end
  limit 1;

  if current_business_id is null or v_member_role not in ('owner', 'manager') then
