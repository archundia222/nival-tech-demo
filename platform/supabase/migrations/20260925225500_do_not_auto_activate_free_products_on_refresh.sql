...
as $$
declare
  v_points_known boolean := false;
  v_intelligence_known boolean := false;
...
begin
  select exists (
    ...
    where e.business_id=p_business_id and e.product_code='nival_points'
  ) or exists (
    select 1 from public.product_subscriptions ps
    where ps.business_id=p_business_id
      and ps.product_code in ('nival_points','nival_points_intelligence','nival_growth_upgrade')
  ) into v_points_known;

  select exists (
    select 1 from public.business_product_entitlements e
    where e.business_id=p_business_id and e.product_code='nival_intelligence'
  ) or exists (
    select 1 from public.product_subscriptions ps
    where ps.business_id=p_business_id
      and ps.product_code in ('nival_intelligence','nival_points_intelligence','nival_growth_upgrade')
  ) into v_intelligence_known;

  if not v_points_known and not v_intelligence_known then
    return;
  end if;

  if v_points_known then
